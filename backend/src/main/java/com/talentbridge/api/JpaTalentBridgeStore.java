...
public class JpaTalentBridgeStore {
    ...
    public RequirementDetailResponse toRequirementDetail(String id) {
        ...
        List<CandidateResponse> candidates = candidateRepo.findByRoleIdIn(roleIds).stream().map(c -> new CandidateResponse(c.getId(), c.getRoleId(), c.getFullName(), c.getEmail(), c.getPhone(), c.getCurrentCompany(), c.getYearsOfExperience(), c.getSource() == null ? null : CandidateSource.valueOf(c.getSource()), PipelineStage.valueOf(c.getCurrentPipelineStage()), c.getResumeFileUrl(), c.getCreatedAt(), c.getUpdatedAt())).collect(Collectors.toList());
    ...
    public RequirementSummaryResponse updateRequirementStatus(String requirementId, RequirementStatus status) {
        Requirement r = requirementRepo.findById(requirementId).orElseThrow(() -> new IllegalArgumentException("Requirement not found"));
        r.setOverallStatus(status.name());
        r.setUpdatedAt(Instant.now());
        requirementRepo.save(r);
        return new RequirementSummaryResponse(r.getId(), r.getCompanyId(), companyRepo.findById(r.getCompanyId()).map(Company::getName).orElse(""), DriveType.valueOf(r.getDriveType()), status, r.isPartialApprovalConfirmed(), r.getCreatedAt(), r.getUpdatedAt(), roleRepo.findByRequirementIdOrderByCreatedAtAsc(r.getId()).size());
    }
    ...
    public CandidateResponse createCandidate(CreateCandidateRequest request) {
        ...
        return new CandidateResponse(c.getId(), c.getRoleId(), c.getFullName(), c.getEmail(), c.getPhone(), c.getCurrentCompany(), c.getYearsOfExperience(), c.getSource() == null ? null : CandidateSource.valueOf(c.getSource()), PipelineStage.valueOf(c.getCurrentPipelineStage()), c.getResumeFileUrl(), c.getCreatedAt(), c.getUpdatedAt());
    }

    public List<CandidateResponse> listCandidatesByRole(String roleId) {
        return candidateRepo.findByRoleIdOrderByUpdatedAtDesc(roleId).stream().map(c -> new CandidateResponse(c.getId(), c.getRoleId(), c.getFullName(), c.getEmail(), c.getPhone(), c.getCurrentCompany(), c.getYearsOfExperience(), c.getSource() == null ? null : CandidateSource.valueOf(c.getSource()), PipelineStage.valueOf(c.getCurrentPipelineStage()), c.getResumeFileUrl(), c.getCreatedAt(), c.getUpdatedAt())).collect(Collectors.toList());
    }
    ...
    public CandidateResponse updateCandidateStage(String candidateId, PipelineStage stage) {
        ...
        c.setUpdatedAt(Instant.now());
        candidateRepo.save(c);
        return new CandidateResponse(c.getId(), c.getRoleId(), c.getFullName(), c.getEmail(), c.getPhone(), c.getCurrentCompany(), c.getYearsOfExperience(), c.getSource() == null ? null : CandidateSource.valueOf(c.getSource()), stage, c.getResumeFileUrl(), c.getCreatedAt(), c.getUpdatedAt());
    }
